...
WORKTREE_ROOT="$(dirname "$MAIN_CHECKOUT")/tasky-worktrees"
# Derive branch prefix from git user name (e.g. "Chris Pyle" → "chris-pyle")
...
generate_traefik_config() {
  local name="$1"
  local project="tasky-${name}"
  mkdir -p "$TRAEFIK_DYNAMIC"
  cat > "$TRAEFIK_DYNAMIC/tasky-${name}.yml" <<YAML
http:
  routers:
    tasky-${name}-api:
      rule: "Host(\`api-tasky-${name}.lvh.me\`)"
      entryPoints:
        - web
      service: tasky-${name}-api
    tasky-${name}-frontend:
      rule: "Host(\`tasky-${name}.lvh.me\`)"
      entryPoints:
        - web
      service: tasky-${name}-frontend

  services:
    tasky-${name}-api:
      loadBalancer:
        servers:
          - url: "http://${project}-api-1:7400"
    tasky-${name}-frontend:
      loadBalancer:
        servers:
          - url: "http://${project}-frontend-1:7401"
YAML
}
...
print_urls() {
  ...
  echo "  Frontend:  http://tasky-${name}.lvh.me"
  echo "  API:       http://api-tasky-${name}.lvh.me"
...
cmd_up() {
  ...
  if [ -f "$wt_path/backend/pyproject.toml" ]; then
    (cd "$wt_path/backend" && uv sync 2>/dev/null || true)
  fi
  ...
  if [ -f "$MAIN_CHECKOUT/backend/.env" ]; then
    echo "Copying backend/.env from main checkout"
    cp "$MAIN_CHECKOUT/backend/.env" "$wt_path/backend/.env"
  else
    echo "Warning: no backend/.env in main checkout ($MAIN_CHECKOUT)"
    echo "  Create one or copy manually to $wt_path/backend/.env"
  fi
  ...
  # 8. Start the stack from the worktree directory
  ...
  while [ $retries -gt 0 ]; do
    local api_healthy
    api_healthy=$(cd "$wt_path" && docker compose -f "$BASE_COMPOSE" -f "$OVERRIDE_FILE" -p "$project" ps api 2>/dev/null | grep -c "healthy" || true)
    if [ "$api_healthy" -gt 0 ]; then
      break
    fi
    retries=$((retries - 1))
    sleep 2
...
cmd_down() {
  ...
  if [ -f "$TRAEFIK_DYNAMIC/tasky-${name}.yml" ]; then
    rm "$TRAEFIK_DYNAMIC/tasky-${name}.yml"
  fi
...
cmd_ls() {
  ...
  for config in "$TRAEFIK_DYNAMIC"/tasky-*.yml; do
    [ -f "$config" ] || continue
    local cfg_name
    cfg_name="$(basename "$config" .yml)"
    cfg_name="${cfg_name#tasky-}"  # strip tasky- prefix
    local cfg_wt_path
    ...
    local cfg_project="tasky-${cfg_name}"
    if is_stack_running "$cfg_project"; then
      echo "  ${cfg_name} (orphaned — no worktree)"
      echo "    Docker:    running"
      echo "    Frontend:  http://tasky-${cfg_name}.lvh.me"
      echo "    API:       http://api-tasky-${cfg_name}.lvh.me"
      echo ""
      found=true
    fi
  done
